...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (dot && ring && window.innerWidth > 768) {
    ...
    document.querySelectorAll('a, button, .strain-product-card, .vault-nav-card, .tier-card, .perk-detail-card').forEach(el => {
  ...
  if (typeof gsap !== 'undefined') {
    ...
    heroTl
      .fromTo('.cat-back-link',
        { opacity: 0, x: -20 },
        { opacity: 1, x: 0, duration: 0.6, ease: 'power3.out' })
      .fromTo('.cat-hero-label',
        { opacity: 0, y: 15 },
        { opacity: 1, y: 0, duration: 0.6, ease: 'power3.out' }, '-=0.3')
      .fromTo('.cat-hero-title',
        { opacity: 0, y: 60, filter: 'blur(8px)' },
        { opacity: 1, y: 0, filter: 'blur(0px)', duration: 1.2, ease: 'power4.out' }, '-=0.4')
      .fromTo('.cat-hero-tagline',
        { opacity: 0, y: 20 },
        { opacity: 1, y: 0, duration: 0.8, ease: 'power3.out' }, '-=0.7')
      .fromTo('.cat-hero-stat',
        { opacity: 0, y: 20 },
        { opacity: 1, y: 0, duration: 0.6, stagger: 0.12, ease: 'power3.out' }, '-=0.5');
    ...
    if (tiersGrid) {
      gsap.from('.tier-card', {
        ...
        scrollTrigger: { trigger: tiersGrid, start: 'top 82%', once: true }
      });
    }

    /* ── Perk detail cards stagger ── */
    const perksGrid = document.querySelector('.perks-detail-grid');
    if (perksGrid) {
      gsap.from('.perk-detail-card', {
        opacity: 0, y: 30, stagger: 0.08, duration: 0.8, ease: 'power3.out',
        scrollTrigger: { trigger: perksGrid, start: 'top 85%', once: true }
      });
    }
    ...
    gsap.from('.lab-metric-card', {
      opacity: 0, scale: 0.92, stagger: 0.1, duration: 0.7, ease: 'back.out(1.5)',
      scrollTrigger: { trigger: '.lab-verify-metrics', start: 'top 85%', once: true }
    });

    /* ── Vault nav cards ── */
    gsap.from('.vault-nav-card', {
      opacity: 0, y: 30, stagger: 0.15, duration: 0.9, ease: 'power3.out',
      scrollTrigger: { trigger: '.vault-nav-grid', start: 'top 85%', once: true }
    });
    ...
    ScrollTrigger.create({
      ...
      onEnter: () => {
        document.querySelectorAll('.flavor-bar-fill').forEach((bar, i) => {
          ...
          gsap.to(bar, { width: tw, duration: 1.4, delay: i * 0.12, ease: 'power3.out' });
        });
      }
    });
    ...
    if (applyCrown) {
      gsap.from(applyCrown, {
        opacity: 0, scale: 0, duration: 0.8, ease: 'back.out(2)',
        scrollTrigger: { trigger: '#apply-section', start: 'top 70%', once: true }
      });
    }
  }
  ...
  const productCards = document.querySelectorAll('.strain-product-card[data-type]');
  ...
  if (sortSelect) {
    sortSelect.addEventListener('change', () => {
      const grid = document.getElementById('strain-cards-grid') || document.getElementById('concentrate-cards-grid');
      if (!grid) return;
      ...
      cards.sort((a, b) => {
        if (val === 'thc') {
          const aThc = parseFloat(a.querySelector('.spc-thc')?.textContent) || 0;
          const bThc = parseFloat(b.querySelector('.spc-thc')?.textContent) || 0;
          return bThc - aThc;
        }
        if (val === 'name') {
        ...
        if (val === 'rarity') {
          ...
          const aRar = rarityScore[a.querySelector('.spc-rarity')?.classList[1]] || 0;
          const bRar = rarityScore[b.querySelector('.spc-rarity')?.classList[1]] || 0;
          return bRar - aRar;
        }
        return 0;
      });
  ...
  /* ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
     VAULT CARD 3D TILT
  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ */
  document.querySelectorAll('.strain-product-card, .vault-nav-card, .tier-card').forEach(card => {
  ...
  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ */
  document.querySelectorAll('.tier-card:not(.tier-featured)').forEach(card => {
